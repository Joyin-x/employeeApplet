...
Page({
    ...
    //同意申请
    agree(e) {
		if (e.currentTarget.dataset.departmentid == this.data.userInfo.department_id){
			this.addOvertime(e.currentTarget.dataset.id, 1);
		}
       else{
			wx.showModal({
				title: '警告',
				content: this.data.userInfo.name + ',你的部门为' + this.data.userInfo.department_name + ',只能审核本部门员工的加班申请',
				showCancel: false,
				confirmText: "返回",
				confirmColor: "#FF1493",
			});
